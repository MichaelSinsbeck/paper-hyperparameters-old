...
for e in errors_raw_fix:
    e_sorted = np.sort(np.array(e), axis = 0)
    this_error = np.exp(np.log( np.array(e)).mean(axis=0))
    errors_fix.append(this_error)

    
for e in errors_raw_re:
    e_sorted = np.sort(np.array(e), axis = 0)
    this_error = np.exp(np.log( np.array(e)).mean(axis=0))
    errors_re.append(this_error)
...
for (e1,e2,n) in zip(errors_fix, errors_re, n_eval_pre):
    prefix = n[0]
    filename = "output/exp1_fig2_{}.data".format(prefix)
    plot_data = np.full((n.size, 3), np.nan)
    plot_data[:,0] = n
    plot_data[:,1] = e1
    plot_data[:,2] = e2
    np.savetxt(filename, plot_data, '%2i %1.3e %1.3e')
